...
public class TestEndpointFluctuations {

	private static final Logger log = LoggerFactory.getLogger(TestEndpointFluctuations.class);

	@Rule
	public WireMockRule wireMockRule = new WireMockRule(7079);

	@Test
	public void testGetDataFluctuations() throws FixerException, ClientProtocolException, IOException { 
		...
		String endpointUrl = "/fluctuation?access_key=" + TestConfig.accessKey + "&base=" + TestConfig.baseCurrency + "&start_date=" + startDate + "&end_date=" + endDate;
		TestUtils.setupMockHttpServer(TestConfig.baseUrl, endpointUrl, jsonStr);

		// With optional parameters
		Endpoint fluctuationEndpoint = new FluctuationEndpoint(TestConfig.baseUrl);
		fluctuationEndpoint.addParam("access_key", TestConfig.accessKey);
		fluctuationEndpoint.addParam("base", TestConfig.baseCurrency);
		fluctuationEndpoint.addParam("start_date", startDate);
		fluctuationEndpoint.addParam("end_date", endDate);
		
		try {
			//ResultHeader data = fluctuationEndpoint.getData();
